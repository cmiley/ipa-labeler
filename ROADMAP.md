...
- [x] **Phase 3 — Inter-annotator agreement** — `agreement.py` pure-Python metrics: frame-level Cohen's κ on 10ms grid, boundary tolerance hit-rate ±50ms (precision A→B, B→A, F1), greedy IoU-matched segment count + length-normalized Levenshtein phoneme error rate. Endpoints `GET /api/clips/<id>/agreement` (per-pair on one clip) and `GET /api/agreement` (corpus-wide pooled frame-κ per user pair). Frontend renders a purple metrics panel in View Others mode showing You-vs-selected-peer headline numbers. 19 tests green (four new IAA tests: identical, disagreement, corpus, two-annotators-required).
